...
**Service media (image, description, Instagram reference).** `POST`/`PUT` `/services` accept `multipart/form-data` with an optional `image` file (`jpg`/`jpeg`/`png`/`webp` only — never `svg` or any other type; max 5MB; 100–4000px on each side), an optional `description` (plain text, max 5000 chars), and an optional `instagram_url` (max 500 chars, see below). Every service response (`show`, `index`, and both write endpoints, plus the customer-facing `GET /branches/{branch}/services`) includes `image_url` — a fully usable public URL (`Storage::disk('public')->url(...)`), never the internal storage path — which is `null` when no image is set. Uploading a new `image` on update replaces and deletes the previous file; sending `remove_image=1` (with no `image` file) clears the image and deletes the stored file without setting a new one — a new file upload always takes precedence over `remove_image` if both are somehow sent together. A service's stored image is only ever deleted on an actual replace/remove, or if the service itself is ever force-deleted (not exposed via any endpoint today — see `App\Models\Service::booted()`); a normal (soft) delete leaves the file in place, matching the rest of this project's soft-delete-is-recoverable convention.

`instagram_url` must be an `https://instagram.com` or `https://www.instagram.com` link to a post, reel, or video — path shaped `/p/{id}`, `/reel/{id}`, or `/tv/{id}` — or it's rejected with a normal `422` (e.g. a non-Instagram domain, or a `javascript:`/other non-`https` scheme). A missing scheme or `http://` is normalized to `https://` and the host is lowercased before validation — a harmless variation like `instagram.com/p/ABC` — but the path/query is never altered, so normalization never changes which post it points to. The backend never downloads, scrapes, or resolves the URL in any way; it is stored and returned as-is. See `App\Support\InstagramUrl`.

**Salon Instagram profile (distinct from a service's `instagram_url` above).** `salons.instagram_url` — the salon's own official Instagram *profile* link, e.g. `https://www.instagram.com/primehairstudio/` — not a specific post/reel. `POST`/`PATCH` `/salon` accept an optional `instagram_url`, exposed on every salon response (`show`, both write endpoints, and the customer-facing `GET /customer/salons`). Validation and normalization reuse the same `App\Support\InstagramUrl` class as the service field (same `https`/`instagram.com`-only/no-scraping rules), but through a separate method, `InstagramUrl::isValidProfile()`: it requires a single path segment shaped like an Instagram username (letters/digits/`.`/`_`, 1–30 chars) and rejects Instagram's own reserved non-profile paths (`p`, `reel`, `tv`, `explore`, etc.) — so a post/reel link is correctly rejected here just as a profile link would be rejected on a service. Sending an empty string clears it (Laravel's `ConvertEmptyStringsToNull` middleware converts it to `null` before validation, same as every other optional salon field) — there is no separate "remove" flag, unlike the service image (which is a file, not a plain string).

**Master catalog & audience segmentation.** New tenants automatically receive a starting service catalog — see `MASTER_CATALOG_ARCHITECTURE.md` for the full design. Services and categories gained an optional `audience` field (`male`/`female`/`unisex`/`kids`), returned on every `Service`/`ServiceCategory` response and filterable via `?audience=` on `GET /services`, `GET /service-categories`, and the customer-facing `GET /branches/{branch}/services` — additive query parameters on existing endpoints, never new ones. `audience` is distinct from the pre-existing `gender` field (`male`/`female`/`unisex` only, kept for backward compatibility) and is the authoritative classification going forward. The master catalog itself (`master_service_categories`/`master_services`) has no HTTP endpoint at all — it is seeder-managed only and copied into a tenant's own services the first time that tenant creates a branch, via `POST /branches` (no new endpoint — the existing branch-creation call now also provisions the catalog server-side, transparently to the client).
