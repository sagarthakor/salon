...
/// Reschedule keeps the booking's existing service/staff composition — the
/// backend only revalidates the new date/time (see BOOKING_ENGINE.md) — so
/// this screen only needs date + slot, not the full service/staff flow.
...
class _RescheduleScreenState extends ConsumerState<RescheduleScreen> {
  ...
  Future<void> _confirmReschedule(Booking booking) async {
    ...
    try {
      await ref
          .read(bookingRepositoryProvider)
          .rescheduleBooking(booking.id, date: toApiDate(_date!), startTime: _selectedSlot!.startTime);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: bookingAsync.when(
        ...
        error: (error, _) => ErrorView(message: 'Could not load this booking.'),
        data: (booking) => ListView(
          ...
          children: [
            ...
            PrimaryButton(
              ...
              onPressed: _selectedSlot != null ? () => _confirmReschedule(booking) : null,
            ),
          ],
        ),
      ),
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return ChoiceChip(
            label: SizedBox(
              width: 40,
              child: Text(day.day.toString(), textAlign: TextAlign.center),
            ),
            selected: isSelected,
